...
export const textToBoldComponents = (text: string, pattern?: string) => {
    ...
    for (const i1 of indices) {
        const i2 = i1 + pattern.length

        tokens.push(
            text.slice(prevIndex, i1),
            <b style={{ color: 'blue' }}>{text.slice(i1, i2)}</b>
        )
...
export const parseStringDate = (dateStr: string): Date | undefined => {
    ...
    // mm/yyyy
        ...
        if (hourRE.test(t)) {
            const hour = parseInt(tokens[3], 10)
            ...
            const hour = parseInt(tokens[3], 10)
            const minute = parseInt(tokens[4], 10)
...
export const handleDateChangeGeneral = (
    ...
    const stateFunc =
        type === DateType.BeginDate ? setBeginDateError : setEndDateError
